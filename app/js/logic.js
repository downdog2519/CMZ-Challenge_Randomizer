...
function buildExtremeQueue() {
    ...
    state.extremeQueue = shuffle(allTypes).slice(0, state.challengeStageCount);
...
function generateTrailChallenge() {
    ...
    // Equal weight across canonical maps (no Astra alias double-count)
    const shuffledMaps = shuffle(trailMaps);

    let chosenMap = null;
    let chosenRelic = null;

    for (const mapName of shuffledMaps) {
        const pool = runMode === 'quickTrail'
            ? (trailMapGrimRelics[mapName] || []).filter((r) => !owned.includes(r))
            : (trailRules[mapName]?.pool || []).filter((r) => !owned.includes(r));
        if (pool.length > 0) {
            chosenMap = mapName;
    ...
    if (!chosenRelic) {
        chosenMap = pickRandom(trailMaps);
    ...
    return {
        ...
        relics: chosenRelic ? [chosenRelic] : [],
        fieldUpgrades: []
    };
}
...
function generateStartingRoomChallenge() {
    ...
    const map   = pickRandom(startingRoomMaps);
    const round = weightedPick(rounds, weights);
...
export function markPass() {
    ...
    updateSessionStats();
    hidePassFail();
    showResultBurst("pass", () => nextStage());
}
...
export function markFail() {
    ...
    updateSessionStats();
    hidePassFail();
    showResultBurst("fail", () => nextStage());
}
...
export function finishRun() {
    ...
    const fails = state.sessionFails;
    const summary = {
        ...
        wins,
        fails,
        outcome: wins > 0 && fails === 0 ? 'perfect'
            : wins >= fails ? 'victory'
            : wins > 0 ? 'mixed'
            : 'defeat',
